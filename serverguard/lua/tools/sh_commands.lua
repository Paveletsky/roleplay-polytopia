...
function command:Execute(player, silent, arguments)
	...
	if (IsValid(target)) then
		if (serverguard.player:HasBetterImmunity(player, serverguard.player:GetImmunity(target))) then
			...
			if (!silent) then
				serverguard.Notify(nil, SERVERGUARD.NOTIFY.GREEN, serverguard.player:GetName(player), SERVERGUARD.NOTIFY.WHITE, " has modeled ", SERVERGUARD.NOTIFY.GREEN, target:Name());
			end
...
command.help		= "Set rank control";
command.command 	= "setrankcon";
command.arguments	= {"player","rank"};
command.permissions	= "Set rank control";
...
function command:Execute(player, silent, arguments)
	...
	if (IsValid(target)) then
		...
		if (!silent) then
				serverguard.Notify(nil, SERVERGUARD.NOTIFY.GREEN, serverguard.player:GetName(player), SERVERGUARD.NOTIFY.WHITE, " выставил звание у ", SERVERGUARD.NOTIFY.GREEN, target:Name(), SERVERGUARD.NOTIFY.WHITE, " на ", SERVERGUARD.NOTIFY.RED, arguments[2]); 
		end;
	end;
end;
...
command.bDisallowConsole	= true;
command.permissions         = "Go Incognito";
